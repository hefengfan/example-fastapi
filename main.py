...
async def periodic_garbage_collection():
    """Background task to periodically clean up memory when API is idle"""
    global last_gc_time
    ...
    while True:
        ...
        # Only run GC if no requests are in progress and interval has passed
        if not request_in_progress and (current_time - last_gc_time) >= gc_interval:
            ...
            try:
                if session_manager.is_initialized():
                    ...
                    current_time = time.time()
                    if current_time - session_manager.last_refresh > session_manager.refresh_interval / 2:
                        logger.info("Proactively refreshing session during idle time")
                ...
                logger.error(f"Error during idle session refresh: {e}")
...
# These functions must not be modified as per user's request
...
# Function to remove "[这是一个数字](@ref)" patterns
def remove_ref_numbers(content: str) -> str:
    """Remove reference number patterns from content"""
    if not content:
        return content
    # Pattern matches [数字](@ref) where 数字 can be any characters
...
def create_chunk(sse_id: str, created: int, content: Optional[str] = None,
    ...
    if content is not None:
        # Clean reference numbers from content
        cleaned_content = remove_ref_numbers(content)
        if is_first:
            delta = {"role": "assistant", "content": cleaned_content}
        else:
...
async def generate_response(messages: List[dict], model: str, temperature: float, stream: bool,
    ...
    try:
        ...
        if close_client:
            await client.aclose()
        request_in_progress = False  # Mark request as completed
        
        # Run garbage collection if it's been a while
        global last_gc_time
        current_time = time.time()
        if current_time - last_gc_time >= gc_interval:
            logger.info("Running post-request garbage collection")
            gc.collect()
            last_gc_time = current_time
...
async def chat_completions(request: ChatCompletionRequest, authorization: str = Header(None), req: Request = None):
    ...
    try:
        ...
        if not request.stream:
            ...
            # Clean reference numbers from final content
            content = remove_ref_numbers(content)
        ...
        # Always mark request as completed when done
        request_in_progress = False
...
async def health_check():
    ...
    if session_manager.is_initialized():
        return {"status": "ok", "session": "active", "last_gc": time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(last_gc_time))}
    else:
        return {"status": "degraded", "session": "inactive", "last_gc": time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(last_gc_time))}
...
async def force_garbage_collection():
    ...
    # Only run if no requests are in progress
    if request_in_progress:
        return {"status": "skipped", "reason": "Request in progress"}
    
    # Run garbage collection
    before_memory = gc.get_count()
    collected = gc.collect(generation=2)  # Full collection
    after_memory = gc.get_count()
    ...
    return {
        ...
        "after": after_memory,
        "timestamp": time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(last_gc_time))
    }
